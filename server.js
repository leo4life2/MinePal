...
function setupVoice(settings, userDataDir, agentProcesses) {
  ...
  if (!input_device_id) {
    ...
    logToFile(
      `Push-to-talk setup skipped. Missing: ${missingItems.join(" and ")}`
    );
    console.log(
      `Push-to-talk setup skipped. Missing: ${missingItems.join(" and ")}`
    );
  ...
  if (key_binding) {
    ...
    try {
      ...
      uIOhook.start();
      logToFile(
        `Push-to-talk enabled with key code: ${keyCode} and input device: ${input_device_id}`
      );
      console.log(
        `Push-to-talk enabled with key code: ${keyCode} and input device: ${input_device_id}`
      );
...
function startServer() {
    ...
    ws.on("message", async (message) => {
      try {
        ...
        if (data.type === "audio-binary") {
          ...
          logToFile(
            `Received audio binary data from client of size ${data.audio.length} bytes`
          );
          ...
          try {
            const model =
              settings.language === "en" || settings.language === "en-US"
                ? "nova-3"
                : "nova-2";
            logToFile(
              `Sending audio to backend for transcription with model: ${model} and language: ${settings.language}`
            );
            ...
            if (transcript) {
              ...
              if (agentProcesses.length > 0) {
                agentProcesses[0].sendMessage(
                  `/tell ${settings.player_username} I couldn't quite catch that, say again?`
                );
              }
            }
          } catch (error) {
            logToFile(
              `Error sending audio for transcription: ${error.message}`
            );
            if (agentProcesses.length > 0) {
              if (
                error.response &&
                ...
                agentProcesses[0].sendMessage(
                  `/tell ${settings.player_username} I couldn't understand that, come again?`
                );
              } else {
                agentProcesses[0].sendMessage(
                  `/tell ${settings.player_username} My voice transcription service took an arrow to the knee, try again later`
                );
              }
            }
            if (error.response) {
  ...
  app.get("/settings", (req, res) => {
    ...
    const ethanTemplate = JSON.parse(
      fs.readFileSync(ethanTemplatePath, "utf8")
    );
  ...
  app.delete("/bot-memories/:botName/:memoryId", async (req, res) => {
    ...
    if (!botName || !memoryId) {
      return res
        .status(400)
        .json({ error: "Bot name and memory ID are required" });
    }
  ...
  app.get("/check-server", (req, res) => {
    ...
    socket
      ...
      .on("error", (err) => {
        logToFile(
          `Server at ${host}:${port} is not reachable. Error: ${err.message}`
        );
  ...
  app.post("/manual-chat", express.json(), (req, res) => {
    ...
    if (!botName || !message) {
      return res
        .status(400)
        .json({ error: "Both 'botName' and 'message' fields are required." });
    }
  ...
  app.post("/start", express.json(), (req, res) => {
    ...
    if (agentProcessStarted) {
      ...
      return res
        .status(409)
        .send("Agent process already started. Restart not allowed.");
    }
    ...
    const emptyFields = Object.entries(newSettings)
      .filter(([key, value]) => {
        ...
        if (
          !newSettings.useOwnApiKey &&
          (key === "openai_api_key" || key === "model")
        ) {
          return false;
        }
        if (key === "profiles")
        ...
        if (key === 'voice_mode') return false;
    ...
    logToFile(
      "API: Settings updated and AgentProcess started for all profiles"
    );
  ...
  app.post("/save-profiles", express.json(), (req, res) => {
    ...
    if (
      ...
      newProfiles.some((profile) => !profile.name || !profile.personality)
    ) {
      return res
        .status(400)
        .json({
          error:
            "Invalid input. Each profile must have 'name' and 'personality' fields.",
        });
    }
